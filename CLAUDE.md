...
- The type drives releases: `feat` bumps the minor version, `fix` the patch. A change to
  workflows, the justfile or tooling is `ci`, `build` or `chore` — never `fix`, even when it
  repairs something, or release-please cuts a release containing no change to the operator.
